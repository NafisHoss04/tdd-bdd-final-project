...
class TestProductRoutes(TestCase):
    ...
    def test_update_product(self):
        ...
        new_product = response.get_json()
        new_product["name"] = "updated"
        response = self.client.put(f"{BASE_URL}/{new_product['id']}", json=new_product)
        self.assertEqual(response.status_code, status.HTTP_200_OK)

        updated_product = response.get_json()
        self.assertEqual(updated_product["name"], "updated")
    ...
    def test_update_product_no_content_type(self):
        ...
        response = self.client.put(f"{BASE_URL}/0", data="bad data")
        self.assertEqual(response.status_code, status.HTTP_415_UNSUPPORTED_MEDIA_TYPE)

    def test_update_product_wrong_content_type(self):
        """It should not Update a Product with wrong Content-Type"""
        response = self.client.put(f"{BASE_URL}/0", data={}, content_type="plain/text")
        self.assertEqual(response.status_code, status.HTTP_415_UNSUPPORTED_MEDIA_TYPE)
